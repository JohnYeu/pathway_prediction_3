...
and scores arbitrary gene lists for pathway-like coherence.
...
seed-42 reference run, XGBoost achieved nested-CV AUROC $0.837 \pm 0.029$
and held-out test AUROC/AUPRC $0.874/0.843$. A supplementary held-out comparison of
...
(test AUROC 0.865--0.879). SHAP analysis identifies
...
Class balance is a second issue. The 1:1 through 1:5 ratios were compared by nested CV within the outer training set. The 1:1 ratio had the highest normalised AUPRC and F1, while its AUROC remained close to the more negative-heavy settings. Reading AUROC together with baseline-adjusted AUPRC, rather than AUROC alone, is what favours the 1:1 setting \citep{saito2015precision}. The outer test set was used only after this choice.
...
Five positive-to-negative ratios ($k \in \{1,2,3,4,5\}$) were compared within the outer training set \citep{japkowicz2002class}. Three repeats of five-fold CV were used. In every fold, controls were generated separately for the fold-training and fold-validation pathways. The variance filter and mutual-information ranking were then fitted on the fold-training records, producing a fold-specific set of 60 GO terms. The outer test set was not used in this comparison. Because the random AUPRC baseline changes with prevalence, raw AUPRC values were also converted to
\begin{equation}
\mathrm{AUPRC}_{\mathrm{norm}} =
\frac{\mathrm{AUPRC}-\pi}{1-\pi}, \qquad \pi=\frac{1}{1+k},
\label{eq:normalized_auprc}
\end{equation}
where $\pi$ is the positive fraction in the validation fold. The 1:1 ratio was used for the final held-out analysis because it had the highest mean normalised AUPRC (0.568) and F1 (0.785). Mean AUROC differed little across ratios: the largest change from 1:1 was 0.012, or 0.54 pooled SD.
...
\midrule
1:1 & $0.837 \pm 0.029$ & 0.784 & 0.500 & $\mathbf{0.568 \pm 0.077}$ & \textbf{0.785} \\
1:2 & $0.831 \pm 0.017$ & 0.647 & 0.333 & $0.470 \pm 0.061$ & 0.662 \\
1:3 & $0.841 \pm 0.018$ & 0.573 & 0.250 & $0.431 \pm 0.076$ & 0.610 \\
1:4 & $0.845 \pm 0.014$ & 0.510 & 0.200 & $0.388 \pm 0.060$ & 0.551 \\
1:5 & $0.848 \pm 0.009$ & 0.461 & 0.167 & $0.353 \pm 0.044$ & 0.514 \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:method_comparison} summarises the held-out performance of all 13 methods on the seed-42 split. The five highest-AUROC models---CatBoost (0.879), XGBoost (0.874), Random Forest (0.874), Gradient Boosting (0.873), and LightGBM (0.865)---are all tree-based. Logistic Regression achieved 0.796 as a linear baseline. Because this comparison uses one outer-test split, it provides model-selection context rather than a definitive algorithm ranking. In the repeated nested-CV analysis of the three reference models, mean AUROC was $0.837 \pm 0.029$ for XGBoost, $0.849 \pm 0.021$ for Random Forest, and $0.801 \pm 0.028$ for Logistic Regression (mean $\pm$ SD over 15 folds).
...
\midrule
1 & CatBoost & 0.879 & 0.855 & 0.793 & 0.139 & 0.44 \\
2 & \textbf{XGBoost} & \textbf{0.874} & \textbf{0.843} & 0.809 & 0.140 & 0.84 \\
3 & Random Forest & 0.874 & 0.841 & 0.828 & 0.145 & 0.44 \\
4 & Gradient Boosting & 0.873 & 0.839 & 0.808 & 0.143 & 0.78 \\
5 & LightGBM & 0.865 & 0.824 & 0.807 & 0.144 & 1.16 \\
6 & AdaBoost & 0.862 & 0.835 & 0.796 & 0.212 & 0.26 \\
7 & Extra Trees & 0.853 & 0.827 & 0.813 & 0.164 & 0.30 \\
8 & RBF SVM & 0.839 & 0.810 & 0.774 & 0.168 & 0.09 \\
9 & Linear SVM & 0.810 & 0.786 & 0.759 & 0.185 & 0.16 \\
10 & k-NN (k=7) & 0.809 & 0.748 & 0.751 & 0.173 & 0.07 \\
11 & Logistic Regression & 0.796 & 0.757 & 0.752 & 0.181 & 0.01 \\
12 & MLP & 0.790 & 0.760 & 0.731 & 0.184 & 0.02 \\
13 & Gaussian Naive Bayes & 0.733 & 0.668 & 0.732 & 0.268 & 0.00 \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{\textbf{ROC and precision--recall curves for the reference XGBoost model} on the seed-42 outer-test split ($n=206$). Left: the ROC curve has AUROC $=0.874$. Right: the precision--recall curve has AUPRC $=0.843$, read against the random baseline of $0.500$ at the 1:1 ratio.}
\label{fig:roc_pr}
\end{figure}

Figure~\ref{fig:roc_pr} shows the two curves behind these summary numbers. The ROC curve reaches an AUROC of $0.874$, and the precision--recall curve gives an AUPRC of $0.843$ above the $0.500$ random baseline. At the default $0.5$ threshold the model gives the confusion matrix in Figure~\ref{fig:confusion}: 89 true positives, 14 false negatives, 75 true negatives, and 28 false positives. Precision is $0.761$, recall is $0.864$, and F1 is $0.809$.
...
Breaking the held-out scores down by gene-set type (Figure~\ref{fig:score_by_type}) shows where the difficulty lies. Random and shuffled controls have low median scores ($0.010$ and $0.007$), while curated pathways have a median of $0.809$. Partial pathways have a median of $0.686$, and cross-pathway mixtures have a median of $0.293$. All 28 false positives come from these two groups: 20 partial and 8 cross-pathway controls. Against random and shuffled controls alone, AUROC is $0.998$ and $0.997$, respectively; it falls to $0.653$ for partial pathways and $0.846$ for cross-pathway mixtures.
...
\subsection{GO feature-selection stability}
...
stable than the term lists ($0.837 \pm 0.029$). The model therefore gives
...
\midrule
1 & \texttt{jaccard\_mean} & Jaccard & 0.732 \\
2 & \texttt{go\_entropy} & size/statistic & 0.560 \\
3 & \texttt{annotated\_gene\_count} & size/statistic & 0.454 \\
4 & \texttt{GO:0008150} & GO term & 0.383 \\
5 & \texttt{jaccard\_std} & Jaccard & 0.326 \\
6 & \texttt{mean\_go\_per\_gene} & size/statistic & 0.323 \\
7 & \texttt{GO:0005737} & GO term & 0.266 \\
8 & \texttt{GO:0009507} & GO term & 0.213 \\
9 & \texttt{GO:0005829} & GO term & 0.178 \\
10 & \texttt{GO:0005739} & GO term & 0.156 \\
\bottomrule
\end{tabular}
\end{table}
...
To measure the contribution of each feature group, we trained XGBoost on subsets of the 69 features using the same held-out split (Table~\ref{tab:s3_ablation}). When any single group is used alone, AUROC drops by 0.064--0.096. When any single group is removed, the drop ranges from 0.025 to 0.036. The full 69-feature model has the highest AUROC, while the remaining groups partly compensate when one group is removed.
...
\midrule
Full model & 69 & 0.874 & 0.843 & 0.000 \\
GO only & 60 & 0.810 & 0.763 & $-$0.064 \\
Jaccard only & 4 & 0.783 & 0.732 & $-$0.091 \\
Size/entropy only & 5 & 0.778 & 0.712 & $-$0.096 \\
$-$GO frequency & 9 & 0.849 & 0.820 & $-$0.025 \\
$-$Jaccard & 65 & 0.838 & 0.815 & $-$0.036 \\
$-$Size/entropy & 64 & 0.848 & 0.777 & $-$0.026 \\
\bottomrule
\end{tabular}
\end{table}
...
The results show that GO-derived features can separate curated \textit{Arabidopsis} pathway instances from constructed controls with moderate-to-good accuracy (test AUROC 0.874, AUPRC 0.843). Some controls still contain real biological signals, making this benchmark harder than using purely random data. The nested-CV and ablation results show that the performance is not explained by one split or one feature group alone.

\subsection{Understanding the performance level}
\label{sec:disc_performance}

The held-out AUROC of 0.874 should be read in light of the benchmark design. If the negative class contained only random gene sets, performance would be higher but biologically less meaningful. The present benchmark mixes four negative types---random, shuffled, partial, and cross-pathway---each testing a different failure mode. Partial pathways retain genuine functional coherence because they are subsets of real pathways. Their median score of 0.686 and negative-type AUROC of 0.653 show that they are the main boundary case. The overall result reflects the difficulty of distinguishing complete curated pathways from partially coherent alternatives.

Raw AUPRC cannot be compared across class ratios without considering its random baseline \citep{saito2015precision}. The ratio comparison was therefore run only within the outer training set, with fold-specific GO selection. The 1:1 ratio had the highest mean normalised AUPRC (0.568) and F1 (0.785). Mean AUROC ranged from 0.831 to 0.848, and the largest difference from 1:1 was only 0.54 pooled SD. Reading the two metrics together favoured 1:1 for the final outer-test analysis. Normalising AUPRC corrects its random baseline, but it does not make precision--recall performance fully invariant to prevalence.

\subsection{Feature-selection variation}
\label{sec:disc_feature_variation}

The selected GO terms changed across CV folds. For the 1:1 analysis, the mean pairwise Jaccard similarity between term sets was 0.123, and only three terms appeared in every fold. The corresponding AUROC SD was 0.029. This suggests that several GO subsets can support similar predictive performance. The selected terms should therefore be read as features of the fitted model, not as a universal biological signature.

\subsection{Interpreting SHAP features and broad GO terms}
\label{sec:disc_shap_interpretation}

The high SHAP importance of \texttt{jaccard\_mean} (0.732) shows that the fitted model relies heavily on pairwise GO overlap. \texttt{go\_entropy} (0.560) reflects whether the selected-GO profile is concentrated on a few terms or spread across many. \texttt{annotated\_gene\_count} (0.454) contributes because small sets provide limited pairwise evidence and very large sets often correspond to broad KEGG overview maps. These values are mean absolute SHAP contributions in model-output units and should be compared within this fitted model rather than interpreted as probabilities.
...
The outer held-out benchmark is free of supervised feature-selection leakage: the variance filter and mutual-information ranking see only the primary training records, and controls are generated separately on the train and test sides. Repeated CV also reruns the label-dependent selection steps inside each fold. However, the low overlap between fold-specific GO lists shows that the feature identities are sensitive to the training sample. Performance is more stable than the term lists, but biological interpretation should focus on recurring feature groups rather than one fixed GO signature.
...
PathwayML-Ath is a supervised framework that scores gene sets for pathway-like coherence in \textit{Arabidopsis thaliana}. The 538 KEGG and AraCyc source records were grouped into 512 unique gene-set modelling instances and encoded as 69-dimensional GO annotation vectors. The XGBoost reference model achieves nested-CV AUROC $0.837 \pm 0.029$ and outer-test AUROC $0.874$ (AUPRC $0.843$). SHAP analysis identifies pairwise GO Jaccard similarity, annotation entropy, and annotated gene count as the main predictors. The score is a prioritisation signal to be considered with ORA/GSEA results, not standalone evidence for pathway discovery. All scripts, generated tables, figures, and provenance metadata are provided in the project archive.
...
The project archive contains generated tables, figures, software versions, and provenance metadata corresponding to the source-file analysis (538 source records, 512 modelling groups, 69 features, XGBoost outer-test AUROC 0.874).
