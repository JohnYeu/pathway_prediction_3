...
seed-42 reference run, XGBoost achieved CV AUROC $0.863 \pm 0.019$
and held-out test AUROC/AUPRC $0.842/0.807$. A supplementary held-out comparison of
13~ML methods (Section~\ref{sec:method_comparison}; Table~\ref{tab:method_comparison}) ranked
Random Forest, XGBoost, LightGBM, CatBoost and Gradient Boosting as the five highest held-out AUROC models
(test AUROC 0.833--0.858). SHAP analysis identifies
...
Class balance is a second issue. The 1:1 positive-to-negative ratio was selected as the main configuration after complete 1:1 through 1:5 pipelines were compared. The balanced branch had the highest normalised AUPRC, with F1 pointing the same way, while its AUROC remained close to the more negative-heavy branches. Reading AUROC together with its baseline-adjusted AUPRC, rather than AUROC on its own, is what favours the 1:1 setting \citep{saito2015precision}. In this thesis, the model comparison and ablation results should be read together, not as isolated tables.
...
The comparison is a supporting analysis, not the headline result. The goal is not to crown the single best algorithm, only to show that the chosen model works well with the current features and data. In these tests the top five models by AUROC---Gradient Boosting, Random Forest, XGBoost, CatBoost, and LightGBM---are all tree-based. That supports using a tree model downstream and indicates the result does not hinge on one algorithm.
...
A three-stage filtering and feature-selection procedure was applied to remove noisy or overly general GO terms while retaining informative features. First, a label-free frequency filter excluded terms annotating fewer than 20 genes or more than 30\% of the 28,998 annotated genes (7,067~$\rightarrow$~829). The positive pathways were then divided into primary training and test sets, and controls were generated independently on each side. Second, a variance filter fitted only to the 860 primary training records removed terms below the 15th percentile of variance (829~$\rightarrow$~704). Finally, scikit-learn's mutual-information estimator \citep{pedregosa2011scikit} ranked the remaining terms against the primary training labels, and the top 60 were kept (704~$\rightarrow$~60). This ranks terms on relevance alone; unlike mRMR~\citep{dingpeng2003}, it does not penalise redundancy between selected terms. The 216 outer-test records were not used for variance filtering or mutual-information ranking. The resulting 60-term representation was fixed for all subsequent analyses of the primary 1:1 branch. Each ratio-comparison branch selected its own 60 terms from its corresponding training records. Cross-validation and LOFO therefore evaluate the fixed primary-training representation rather than repeating feature selection within each fold.
...
The 538 curated pathways constitute the positive class ($y = 1$), giving $n_+ = 538$. Before source-derived controls were created, positive pathways were split 80:20 with random seed 42 and stratification by database source (KEGG or AraCyc), producing 430 training positives and 108 test positives. Training controls were generated only from training pathways and test controls only from test pathways. At the primary 1:1 ratio this yielded 430 training controls and 108 test controls, for $n_{\mathrm{train}}=860$ and $n_{\mathrm{test}}=216$. Genes may occur on both sides because curated pathways overlap and random controls share a common annotated-gene background; the prohibited dependency is a source pathway used to construct a control on the opposite side.
...
Five positive-to-negative ratios ($k \in \{1,2,3,4,5\}$) were evaluated to assess sensitivity to class balance \citep{japkowicz2002class}. Each ratio was treated as a complete analysis branch: controls were regenerated, 60 GO terms were selected from that branch's training data, and XGBoost was refitted before held-out evaluation. Because the random AUPRC baseline changes with prevalence, raw AUPRC values were also converted to
\begin{equation}
\mathrm{AUPRC}_{\mathrm{norm}} =
\frac{\mathrm{AUPRC}-\pi}{1-\pi}, \qquad \pi=\frac{1}{1+k},
\label{eq:normalized_auprc}
\end{equation}
where $\pi$ is the positive fraction in the test branch. The 1:1 branch ($n_+=538$, $n_-=538$, $n_{\mathrm{total}}=1{,}076$) was selected as the primary configuration. It gave the highest normalised AUPRC (0.614), with its F1 (0.802) likewise the highest, while its AUROC (0.842) was only 0.012 lower than the 1:2 result.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Fig7_robustness.png}
\caption{\textbf{Positive-to-negative ratio comparison (1:1 to 1:5).}
Each point comes from an independently constructed branch with its own training-selected GO representation and fitted XGBoost model. The blue line is held-out AUROC; the red line is AUPRC after adjustment for the positive-class baseline (Equation~\ref{eq:normalized_auprc}). The 1:1 branch gives the highest normalised AUPRC, whereas AUROC rises gradually as more controls are added.}
\label{fig:robustness}
\end{figure}

\begin{table}[H]
\centering
\caption{\textbf{Performance across independently fitted positive-to-negative ratio branches.}
The random AUPRC baseline equals the positive fraction in each held-out set.}
...
pathway-level split ($n_{\mathrm{train}} = 860$; $n_{\mathrm{test}} = 216$;
...
The mean Jaccard statistic captures average similarity among genes, while the minimum flags whether any gene pair is functionally far apart. The maximum and standard deviation then describe local high-similarity structure and whether similarity is uniform or concentrated in a subset of pairs. Together, these features provide a compact summary of internal GO-based topology. The approach is graph-free because the graph is not an external interaction network; it is an implicit similarity structure induced by GO overlap.
...
Table~\ref{tab:method_comparison} summarises the held-out performance of all 13 methods on the seed-42 split. The five highest-AUROC models---Random Forest (0.858), XGBoost (0.842), LightGBM (0.841), CatBoost (0.837), and Gradient Boosting (0.833)---are all tree-based, matching the broader finding that ensemble and boosting methods handle non-linear feature interactions well on tabular biological data. Logistic Regression achieved 0.800 as a linear baseline. Because this comparison uses a single held-out split, it serves as model-selection context rather than a definitive algorithm ranking. In the repeated source-isolated CV analysis of the three reference models, mean AUROC was $0.863 \pm 0.019$ for XGBoost, $0.873 \pm 0.012$ for Random Forest, and $0.837 \pm 0.016$ for Logistic Regression (mean $\pm$ SD over 15 folds under the fixed primary-training representation).
...
\midrule
1 & Random Forest & 0.858 & 0.802 & 0.828 & 0.148 & 0.42 \\
2 & \textbf{XGBoost} & \textbf{0.842} & \textbf{0.807} & 0.802 & 0.156 & 0.86 \\
3 & LightGBM & 0.841 & 0.803 & 0.796 & 0.157 & 1.19 \\
4 & CatBoost & 0.837 & 0.796 & 0.812 & 0.160 & 0.46 \\
5 & Gradient Boosting & 0.833 & 0.795 & 0.784 & 0.164 & 0.87 \\
6 & Extra Trees & 0.832 & 0.789 & 0.794 & 0.167 & 0.30 \\
7 & AdaBoost & 0.820 & 0.746 & 0.788 & 0.209 & 0.29 \\
8 & RBF SVM & 0.804 & 0.749 & 0.745 & 0.177 & 0.10 \\
9 & Linear SVM & 0.802 & 0.737 & 0.781 & 0.183 & 0.22 \\
10 & Logistic Regression & 0.800 & 0.741 & 0.775 & 0.179 & 0.01 \\
11 & MLP & 0.767 & 0.706 & 0.757 & 0.191 & 0.03 \\
12 & Gaussian Naive Bayes & 0.716 & 0.651 & 0.731 & 0.330 & 0.00 \\
13 & k-NN (k=7) & 0.700 & 0.639 & 0.710 & 0.225 & 0.04 \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{\textbf{ROC and precision--recall curves for the reference XGBoost model} on the seed-42 held-out split ($n=216$). Left: the ROC curve plots recall against the false-positive rate; the area under it (AUROC $=0.842$) is the probability that a random curated pathway scores above a random control. Right: the precision--recall curve plots precision against recall; its area (AUPRC $=0.807$) is read against the random baseline of $0.500$, which equals the positive fraction at the 1:1 ratio.}
\label{fig:roc_pr}
\end{figure}

Figure~\ref{fig:roc_pr} shows the two curves behind these summary numbers. The ROC curve reaches an AUROC of $0.842$, so a randomly chosen curated pathway is ranked above a randomly chosen control about 84\% of the time. The precision--recall curve gives an AUPRC of $0.807$ above the $0.500$ random baseline. At the default $0.5$ threshold the model gives the confusion matrix in Figure~\ref{fig:confusion}, with 93 true positives and 15 false negatives among the 108 held-out pathways, and 77 true negatives and 31 false positives among the 108 controls---a precision of $0.750$ and a recall of $0.861$, which give an F1 of $0.802$.

\begin{figure}[H]
\centering
\includegraphics[width=0.56\textwidth]{FigF_confusion.png}
\caption{\textbf{Confusion matrix for the reference XGBoost model} at the default $0.5$ threshold on the 216 held-out gene sets (108 curated pathways and 108 constructed controls).}
\label{fig:confusion}
\end{figure}

Breaking the held-out scores down by gene-set type (Figure~\ref{fig:score_by_type}) shows where the difficulty lies. Random and shuffled controls are given very low scores (median $0.008$ and $0.007$), while curated pathways have a median score of $0.807$. The two constructed hard negatives fall nearer the threshold: partial pathways have a median of $0.661$, and cross-pathway mixtures have a median of $0.349$. All 31 false positives come from these two groups (19 partial and 12 cross-pathway controls), which is why the benchmark is best read as a stress test rather than as an easy positive-versus-random contrast.

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{FigG_score_by_type.png}
\caption{\textbf{Held-out model score by gene-set type.} Box plots (with individual points) of the reference XGBoost score for the 216 held-out sets, split by the curated positives and the four constructed negative types. Random and shuffled controls receive low scores, whereas partial pathways and cross-pathway mixtures---the deliberately hard negatives---lie closer to the $0.5$ threshold (dashed line).}
...
Mean LOFO AUROC was $0.857 \pm 0.036$ (range 0.804--0.903; Figure~\ref{fig:lofo}B; Table~\ref{tab:lofo}). The highest per-family AUROC values were AraCyc\_AminoAcid ($0.903$), KEGG\_Metabolism ($0.901$), and AraCyc\_Lipid ($0.890$); the lowest was KEGG\_Cellular ($0.804$). All 12 evaluated families remain above chance under the fixed primary-training representation. The variation across families reflects differences in annotation density, pathway size distributions, biological complexity, and the non-nested feature-selection protocol.
...
(B)~XGBoost test AUROC when each family is held out. Dashed line: random baseline (AUROC = 0.500). Mean AUROC $=0.857 \pm 0.036$ across 12 evaluated families under the fixed primary-training feature representation.}
\label{fig:lofo}
\end{figure}
...
Each row holds out one broad pathway family as the positive test category; all pathways from that family are removed from model training and used as the held-out positive set. Negative samples are regenerated independently from each fold side. Overall mean AUROC $=0.857 \pm 0.036$ (range 0.804--0.903).}
\label{tab:lofo}
\small
...
\midrule
AraCyc\_AminoAcid & 22 & 18 & \textbf{0.903} & 0.899 & 0.809 \\
KEGG\_Metabolism & 9 & 71 & \textbf{0.901} & 0.869 & 0.857 \\
AraCyc\_Lipid & 23 & 12 & \textbf{0.890} & 0.898 & 0.833 \\
AraCyc\_Nucleotide & 20 & 18 & \textbf{0.885} & 0.893 & 0.722 \\
KEGG\_SignalTrans & 25 & 55 & \textbf{0.882} & 0.852 & 0.756 \\
KEGG\_GeneticInfo & 21 & 61 & \textbf{0.873} & 0.840 & 0.851 \\
AraCyc\_Misc & 286 & 11 & \textbf{0.851} & 0.806 & 0.792 \\
KEGG\_Lipid & 27 & 30 & \textbf{0.834} & 0.859 & 0.692 \\
AraCyc\_Carbohydrate & 15 & 27 & \textbf{0.831} & 0.786 & 0.750 \\
AraCyc\_Cofactor & 13 & 10 & \textbf{0.822} & 0.777 & 0.828 \\
AraCyc\_Secondary & 4 & 25 & \textbf{0.812} & 0.893 & 0.750 \\
KEGG\_Cellular & 71 & 41 & \textbf{0.804} & 0.729 & 0.792 \\
\midrule
\textbf{Mean $\pm$ SD} & & & $\mathbf{0.857 \pm 0.036}$ & $0.842 \pm 0.056$ & $0.786 \pm 0.052$ \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
1 & \texttt{jaccard\_mean} & Jaccard & 0.919 \\
2 & \texttt{go\_entropy} & size/statistic & 0.655 \\
3 & \texttt{pathway\_size} & size/statistic & 0.554 \\
4 & \texttt{mean\_go\_per\_gene} & size/statistic & 0.276 \\
5 & \texttt{GO:0008150} & GO term & 0.240 \\
6 & \texttt{GO:0009507} & GO term & 0.238 \\
7 & \texttt{GO:0003729} & GO term & 0.212 \\
8 & \texttt{jaccard\_max} & Jaccard & 0.206 \\
9 & \texttt{jaccard\_std} & Jaccard & 0.199 \\
10 & \texttt{GO:0005515} & GO term & 0.181 \\
\bottomrule
\end{tabular}
\end{table}
...
To measure the contribution of each feature group, we trained XGBoost on subsets of the 69 features using the same held-out split (Table~\ref{tab:s3_ablation}). When any single group is used alone, AUROC drops by 0.066--0.079, showing that no single group is sufficient. When any single group is removed, the AUROC drop ranges from 0.008 to 0.043, indicating that the remaining groups partially compensate. The full 69-feature model has the highest AUROC among these configurations, while the smaller effect of removing the size/entropy group shows that much of its signal overlaps with GO frequency and Jaccard features.
...
\midrule
Full model & 69 & 0.842 & 0.807 & 0.000 \\
GO only & 60 & 0.776 & 0.717 & $-$0.066 \\
Jaccard only & 4 & 0.763 & 0.720 & $-$0.079 \\
Size/entropy only & 5 & 0.768 & 0.676 & $-$0.074 \\
$-$GO frequency & 9 & 0.824 & 0.773 & $-$0.018 \\
$-$Jaccard & 65 & 0.799 & 0.755 & $-$0.043 \\
$-$Size/entropy & 64 & 0.834 & 0.784 & $-$0.008 \\
\bottomrule
\end{tabular}
\end{table}
...
The results show that GO-derived features can separate curated \textit{Arabidopsis} pathways from constructed controls with moderate-to-good accuracy (test AUROC 0.842, AUPRC 0.807). The performance is not perfect, and that is expected. Some controls still contain real biological signals, making this benchmark harder than using purely random data. That AUROC remains above chance in all 12 evaluated LOFO families, together with the ablation results, is consistent with the model using transferable coherence signals under the fixed primary-training representation.

\subsection{Understanding the performance level}
\label{sec:disc_performance}

The held-out AUROC of 0.842 should be read in light of the benchmark design. If the negative class contained only random gene sets, performance would likely be higher but biologically less meaningful. The present benchmark mixes four negative types---random, shuffled, partial, and cross-pathway---each testing a different failure mode. Partial pathways, in particular, retain genuine functional coherence because they are subsets of real pathways; a model that assigns them intermediate scores is behaving reasonably, even though this lowers overall AUROC relative to an easy-negative benchmark. The observed performance therefore reflects both the difficulty of distinguishing complete curated pathways from partially coherent alternatives and the limits of the current feature representation.

Raw AUPRC cannot be compared across class ratios without considering its random baseline \citep{saito2015precision}. Across the five independently fitted branches, AUROC rose from 0.842 at 1:1 to 0.874 at 1:5, but raw AUPRC fell from 0.807 to 0.537 as the positive fraction decreased. After baseline adjustment, the 1:1 branch had the highest normalised AUPRC (0.614), and its F1 (0.802) was the highest as well. We therefore selected 1:1 for the formal analysis: it preserves candidate-level precision and recall, while giving up only 0.012 AUROC relative to 1:2. This choice does not mean that 1:1 is best for every use case. A screening application with many more controls may prefer an imbalanced branch and a different threshold. In all cases, users should treat the model score as a ranking tool rather than a calibrated probability, and combine it with SHAP inspection and enrichment context before committing to experimental follow-up.

\subsection{LOFO family variation}
\label{sec:disc_lofo_variation}

The LOFO results (mean AUROC $0.857 \pm 0.036$) provide preliminary evidence of cross-family discrimination under the fixed primary-training representation, but the spread across families is informative. AraCyc\_AminoAcid achieves the highest AUROC (0.903), followed by KEGG\_Metabolism (0.901) and AraCyc\_Lipid (0.890). At the other end, KEGG\_Cellular (0.804), AraCyc\_Secondary (0.812), and AraCyc\_Cofactor (0.822) are harder to distinguish. Differences in family size, pathway breadth, and GO annotation patterns may all contribute; because feature selection is not nested within LOFO, these values should not be interpreted as unbiased estimates for wholly unseen biological families.
...
The high SHAP importance of \texttt{jaccard\_mean} (0.919) shows that the fitted model relies heavily on pairwise GO overlap, consistent with the design intention that this overlap captures a meaningful aspect of pathway-like coherence. \texttt{go\_entropy} (0.655) reflects whether the selected-GO profile is concentrated on a few terms or spread across many. \texttt{pathway\_size} (0.554) contributes because very small sets provide limited pairwise evidence and very large sets often correspond to broad KEGG overview maps that behave differently from specific metabolic or signalling pathways. These values are mean absolute SHAP contributions in model-output units and should be compared within this fitted model rather than interpreted as probabilities.

Among the GO term features, GO:0008150 (biological process) ranks fifth, GO:0009507 (chloroplast) ranks sixth, and GO:0003729 (mRNA binding) ranks seventh. GO:0008150 is broad, and its SHAP contribution does not mean that ``being annotated to biological process'' is biologically specific in itself; it may partly proxy annotation depth. When using SHAP to interpret individual candidates, specific GO terms and Jaccard features should therefore be considered alongside broad terms and size-related features. A high score driven mainly by broad annotation or gene-set size still requires additional biological evidence.
...
PathwayML-Ath is a supervised framework that scores gene sets for pathway-like coherence in \textit{Arabidopsis thaliana}. Trained on 538 KEGG and AraCyc pathways and encoded as a 69-dimensional GO annotation feature vector, the XGBoost reference model achieves CV AUROC $0.863 \pm 0.019$ and held-out test AUROC $0.842$ (AUPRC $0.807$). SHAP analysis identifies pairwise GO Jaccard similarity, annotation entropy, and gene-set size as the main predictors, consistent with the hypothesis that curated pathways share a measurable internal functional structure. It works best as a prioritisation signal weighed together with ORA/GSEA results, not as standalone evidence for pathway discovery. All scripts, generated tables, figures, and provenance metadata are provided in the project archive.
...
The project archive contains generated tables, figures, software versions, and provenance metadata corresponding to the source-file analysis (538 pathways, 69 features, XGBoost held-out AUROC 0.842).
