...
The comparison is a supporting analysis, not the headline result. The goal is not to crown the single best algorithm, only to show that the chosen model works well with the current features and data. In these tests the top five models by AUROC---CatBoost, XGBoost, Random Forest, Gradient Boosting, and LightGBM---are all tree-based. That supports using a tree model downstream and indicates the result does not hinge on one algorithm.
...
Breaking the held-out scores down by gene-set type (Figure~\ref{fig:score_by_type}) shows where the difficulty lies. Median scores are lowest for random and shuffled controls ($0.010$ and $0.007$) and highest for curated pathways ($0.809$); the two hard negatives fall in between, at $0.686$ for partial pathways and $0.293$ for cross-pathway mixtures. All 28 false positives come from these two groups: 20 partial and 8 cross-pathway controls. Against random and shuffled controls alone, AUROC is $0.998$ and $0.997$, respectively; it falls to $0.653$ for partial pathways and $0.846$ for cross-pathway mixtures.
...
To measure the contribution of each feature group, we trained XGBoost on subsets of the 69 features using the same held-out split (Table~\ref{tab:s3_ablation}). Using any one group alone costs 0.064--0.096 AUROC, whereas dropping a single group costs only 0.025--0.036. The full 69-feature model has the highest AUROC, with the surviving groups partly covering for whichever one is left out.
...
\texttt{jaccard\_mean} dominates the SHAP ranking (0.732), confirming that the fitted model leans heavily on pairwise GO overlap. Behind it, \texttt{go\_entropy} (0.560) tracks whether the selected-GO profile is concentrated on a few terms or spread across many, and \texttt{annotated\_gene\_count} (0.454) matters because small sets provide limited pairwise evidence while very large sets often correspond to broad KEGG overview maps. These values are mean absolute SHAP contributions in model-output units and should be compared within this fitted model rather than interpreted as probabilities.
...
PathwayML-Ath is a supervised framework that scores gene sets for pathway-like coherence in \textit{Arabidopsis thaliana}. The 538 KEGG and AraCyc source records were grouped into 512 unique gene-set modelling instances and encoded as 69-dimensional GO annotation vectors. The XGBoost reference model achieves nested-CV AUROC $0.837 \pm 0.029$ and outer-test AUROC $0.874$ (AUPRC $0.843$). SHAP analysis identifies pairwise GO Jaccard similarity, annotation entropy, and annotated gene count as the main predictors. The score is meant to prioritise candidates for follow-up, read together with ORA/GSEA results; on its own it is not evidence of a new pathway. All scripts, generated tables, figures, and provenance metadata are provided in the project archive.
